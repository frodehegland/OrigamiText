...
// MARK: - Copy to Cite: the three-flavour citation clipboard

// The OrigamiCitation value itself lives in OrigamiReading.swift — it is
// pure Foundation and the visionOS target needs it; only this AppKit
// clipboard stays Mac-side.
...
enum CitationClipboard {
    ...
    static func htmlDocument(for citation: OrigamiCitation) -> String {
    ...
    static func html(for citation: OrigamiCitation) -> String {
        // Primary href is origamitext:// — opens Origami Text directly.
        // Author's HTML fallback parser also accepts this form.
        "<a href=\"\(citation.url)\">\(htmlEscaped(citation.marker))</a>"
    }
    ...
    private static func fromHTML(_ html: String) -> OrigamiCitation? {
        ...
        return OrigamiCitation(to: link.to, fragment: link.fragment, rel: "cites",
                               ...
                               documentTitle: nil, documentFilename: nil)
    }
