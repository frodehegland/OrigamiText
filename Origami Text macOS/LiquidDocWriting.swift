...
nonisolated enum ACMLaTeX {
    ...
    static func bundle(for source: LiquidDoc, style: Style = .sigconf,
                       rights: Rights? = nil, event: Conference? = nil,
                       colophon: Bool = true) -> Bundle {
        let doc = colophon ? source : withoutColophon(source)
        var images: [String: Data] = [:]
    ...
    static func latex(for source: LiquidDoc, style: Style = .sigconf,
                      rights: Rights = .ccBy, event: Conference? = nil,
                      colophon: Bool = false) -> String {
        ...
        if colophon, !hasColophon(doc) {
            out.append(contentsOf: generatedColophon(for: doc))
        }
        out.append("")
    ...
    // MARK: The Visual-Meta colophon

    /// The body's Visual-Meta colophon, when the paper carries one: a
    /// heading naming Visual-Meta, and everything under it until a
    /// heading of the same rank or higher.
    static func colophonRange(in body: [LiquidDoc.Paragraph]) -> Range<Int>? {
        guard let start = body.firstIndex(where: { paragraph in
            guard paragraph.heading != nil else { return false }
            let text = paragraph.text.lowercased()
            return text.contains("visual-meta") || text.contains("visual meta")
        }), let level = body[start].heading else { return nil }
        let end = body[(start + 1)...].firstIndex { ($0.heading ?? Int.max) <= level } ?? body.count
        return start..<end
    }

    static func hasColophon(_ doc: LiquidDoc) -> Bool {
        colophonRange(in: doc.body ?? []) != nil
    }

    /// The paper without its Visual-Meta colophon — the export sheet's
    /// "Include the Visual-Meta colophon" left off.
    static func withoutColophon(_ source: LiquidDoc) -> LiquidDoc {
        var doc = source
        guard var body = doc.body, let range = colophonRange(in: body) else { return doc }
        body.removeSubrange(range)
        doc.body = body
        return doc
    }

    /// A Visual-Meta colophon for a paper that has none: the paper's own
    /// citation as BibTeX, written on the page between the markers, so
    /// the PDF carries its metadata in words a person and a machine can
    /// both read — the Visual-Meta approach.
    static func generatedColophon(for doc: LiquidDoc) -> [String] {
        ...
        if let isbn = doc.isbn, !isbn.isEmpty { fields.append(("isbn", isbn)) }
        if !doc.keywords.isEmpty { fields.append(("keywords", doc.keywords.joined(separator: ", "))) }
        let family = names.first.map { name -> String in
        ...
        return [
            "",
            "\\section*{Visual-Meta}",
            "This paper carries its own metadata as Visual-Meta: the citation "
                + "below is how it asks to be cited, written in the document itself "
                + "so it survives printing, copying and conversion "
...
extension ACMLaTeX {
    ...
    static func bundle(for source: LiquidDoc, publisher: Publisher, style: Style = .sigconf,
                       rights: Rights? = nil, event: Conference? = nil,
                       colophon: Bool = true) -> Bundle {
        guard publisher != .acm else {
            return bundle(for: source, style: style, rights: rights, event: event,
                          colophon: colophon)
        }
        let doc = colophon ? source : withoutColophon(source)
        var images: [String: Data] = [:]
    ...
    static func publisherLaTeX(for source: LiquidDoc, publisher: Publisher,
                               rights: Rights, event given: Conference?,
                               colophon: Bool = false) -> String {
        let doc = withFrontMatterFromBody(source)
