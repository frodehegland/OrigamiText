...
- `sourceURL` — where the document **came from**, when it was imported from an online system: the canonical origin URL as a string — a Seed hypermedia address (`hm://<uid>/<path>` or `https://<host>/hm/<uid>/<path>`), a web page. Omit rather than emit an empty string; preserve verbatim on round trip. When present, a citation copied out of the document also names this origin as a rendition source (`vm-source-seed` / `vm-source-web` in the citation's BibTeX), with the paragraph fragment carried over — for Seed-imported documents the paragraph IDs *are* the Seed block IDs, so the same fragment anchors both renditions.
