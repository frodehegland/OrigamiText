...
nonisolated enum ACMLaTeX {
    ...
    /// acmart's own template names. `sigconf` is the two-column
    /// conference proceedings format — what HT papers are set in.
    enum Style: String, Sendable, CaseIterable {
        case sigconf, sigplan, acmsmall, acmlarge, manuscript

        var label: String {
            switch self {
            case .sigconf: "ACM conference, two column"
            case .sigplan: "ACM SIGPLAN, two column"
            case .acmsmall: "ACM journal, small"
            case .acmlarge: "ACM journal, large"
            case .manuscript: "Manuscript, one column, for review"
            }
        }
    }
    ...
    static func latex(for doc: LiquidDoc, style: Style = .sigconf) -> String {
        var out: [String] = [
        ...
        out.append(contentsOf: preamble(for: doc))
    ...
    private static func preamble(for doc: LiquidDoc) -> [String] {
        var out: [String] = []
        ...
        if let uri = doc.licenseURI, uri.contains("creativecommons.org") {
            out.append("\\setcopyright{cc}")
            out.append("\\copyrightyear{\(year(of: doc))}")
            out.append("\\acmYear{\(year(of: doc))}")
            if let code = creativeCommonsCode(for: uri) {
                out.append("% licence: \(code)")
            }
        }
        if let doi = doc.doi, !doi.isEmpty {
            out.append("\\acmDOI{\(escaped(doi))}")
        }
        if let isbn = doc.isbn, !isbn.isEmpty {
        ...
        for paragraph in doc.body ?? [] {
            ...
            if let level = paragraph.heading {
                ...
                let command = ["section", "section", "subsection", "subsubsection"][
                    min(max(level, 1), 3)]
                out.append("")
                out.append("\\\(command){\(inline(text, in: doc))}")
    ...
    private static func isFrontMatter(_ paragraph: LiquidDoc.Paragraph,
        ...
        if let abstract = doc.abstract, !abstract.isEmpty, text == abstract { return true }
    ...
    static func inline(_ text: String, in doc: LiquidDoc) -> String {
        ...
        out = replacing(#"\[i?note:([^\]]+)\]"#, in: out) { groups in
            ...
            return "\\footnote{\(inline(note.text, in: doc))}"
        }
