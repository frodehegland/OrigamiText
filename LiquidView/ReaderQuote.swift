...
enum CitationClipboard {
    ...
    static func write(_ citation: OrigamiCitation) {
        ...
        // The visible form everywhere is the compact marker; the quote and
        // address ride in the link. Flavours:
        //  • private JSON — native Origami/Author paste (exact origamitext:// id)
        //  • RTF — generated by AppKit from an attributed string with a `.link`,
        //    i.e. byte-for-byte the form TextEdit puts on the clipboard, which
        //    Pages and Word both honour on paste and carry into .docx. (A
        //    hand-built HYPERLINK-field RTF is NOT enough — Pages' importer
        //    is picky about the exact form.)
        //  • HTML document — web editors and the fallback recoverer
        //  • plain string — last resort
        pasteboard.setString(citation.marker, forType: .string)
        pasteboard.setString(htmlDocument(for: citation), forType: .html)
        if let rtf = rtf(for: citation) {
